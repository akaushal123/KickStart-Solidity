...
describe('Campaign',()=>{
    ...
    it('allows manager to make payment request', async()=>{
       await campaign.methods.createRequest('Buy Battery', '200', accounts[2]).send({
            from: accounts[0],
           gas: '1000000'
       });
       const request = await campaign.methods.requests(0).call();
       assert.strictEqual(request.description, 'Buy Battery');
    });
    ...
    it('processes request', async()=>{
        ...
        await campaign.methods
            .createRequest('Buy Printer', web3.utils.toWei('5', 'ether'), accounts[1]).send({
            from: accounts[0],
            gas: '1000000'
        });
